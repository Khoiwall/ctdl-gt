...
//###INSERT CODE HERE -
node *_insert(node* &p, int number){
    if (p == NULL){
        p = new node;
        p->data = number;
        p->left = p->right = NULL;
...
node *_delete(node* &p, int number){
    ...
    if (number < p->data){
        p->left = _delete(p->left, number);
    }
    else if (number > p->data){
        p->right = _delete(p->right, number);
    }
    else{
        if (p->left == NULL){
            node *temp = p->right;
        ...
        p->right = _delete(p->right, temp->data);
    }
    return p;
}
